  ...
  (let* ((url (if (and (stringp id-or-url) (string-prefix-p "http" id-or-url))
    ...
    (let-alist info
      ...
      (let* ((title .roomStore.roomInfo.room.title)
             ...
             (urls (cl-loop for v in '(ld sd hd origin)
                            for V = (pcase v ('ld "标清") ('sd "高清") ('hd "超清") ('origin "蓝光"))
                            for s = (alist-get 'main (alist-get v stream))
                            when s
                            collect (propertize (alist-get 'flv s) 'line-prefix (format "[flv/%s] " V)) and
                            collect (propertize (alist-get 'hls s) 'line-prefix (format "[hls/%s] " V)))))
        (if (called-interactively-p 'any)
