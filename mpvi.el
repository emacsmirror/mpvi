  ...
  (let ((seek (lambda ()
    ...
    (if (and (equal path current) (if emmsp (eq type 'emms) (not (eq type 'emms))))
      ...
      (let* ((lowp (ignore-errors (mpvi-compare-mpv-version #'< '(0 38 0))))
             ...
             (items (mapcar #'ensure-list (plist-get info :items)))
             (index (or (plist-get info :index)
                        (if-let* ((p (mpvi-get 'path))) (cl-position p items :key #'car :test #'equal))
                        0))
             (item (plist-get info :item)))
        (when (and items (not addp))
          ;; choose from playlist interactively
          (let* ((cs (cl-loop for c in items for i from 1
                              for s = (propertize (format "%d:%s:%s" i (car c) (or (cdr c) ""))
                                                  'display (concat (propertize (format "%s " i) 'face 'font-lock-comment-face)
                                                                   (or (cdr c) (car c))))
                              collect (list s (car c) (cdr c))))
                 (cs (append (cl-subseq cs index) (cl-subseq cs 0 index)))
                 (f1 (lambda (c)
                       (when-let* ((cand (assoc c cs)) (u1 (cadr cand)) (t1 (caddr cand)))
                         (concat (make-string (max 2 (- 60 (string-width t1))) ? )
                                 (substring-no-properties u1)))))
                 (f2 (lambda (input pred action)
        ...
        ;; only change playback position when start the same item
        (if (and item (equal item current)) (funcall seek)
          (let* ((items (if item `((,item)) items))
                 ...
                 (ytdl-opt (when mpvi-ytdl-extra-options
                             (mapconcat #'identity
                 ...
                 (play-cmd nil))
            (if addp
                ;; add items to playlist, if player is not playing, play the first
                (setq play-cmd
                      `(batch
                        ,@init-cmds
                        ,@(cl-loop for c in items for i from 0
                                   for flag = (if (eq type 'playlist)
                                                  (if (zerop i)
                                                      (if current-prefix-arg 'insert-next-play 'append-play)
                                                    (if current-prefix-arg 'insert-next 'append))
                                                type)
                                   for title = (cdr c)
                                   for opts = (funcall join (if title (cons (cons 'force-media-title title) load-opts) load-opts))
                                   ...
                                   for cmd = `((loadfile ,(car c) ,(intern-soft flag)
                                                         ,@(if (and opts (not lowp)) (list -1))
                                                         ,@(if opts (list opts))))
                                   collect cmd)))
              ;; play the item directly
              (setq load-opts
                    (funcall join
                             (append `((start . ,beg))
                                     (when end     `((ab-loop-a . ,beg) (ab-loop-b . ,end)))
                                     (when title   `((force-media-title . ,(format "\"%s\"" title))))
                                     (when subfile `((sub-file . ,subfile)))
                                     load-opts)))
              (setq play-cmd
                    `(batch
                      ,@init-cmds
                      ...
                      ((loadfile ,item replace
                                 ,@(if (and load-opts (not lowp)) (list -1))
                                 ,@(if load-opts (list load-opts)))
                       .
                       ,(lambda (_ err)
                          ...
                          (if start-func
                              (funcall start-func)
                            (message "%s" (if title
                                              (concat (if logo (format "/%s:" logo))
                                                      (propertize title 'face 'font-lock-keyword-face))
                                            ""))))))))
            (let ((launcher (lambda ()
                              (setq mpvi-current-playing info)
            ...
            (setq info
                  `(,@info :items
                           ,(cl-loop for item across (alist-get 'entries json)
                                     ...
                                     collect (list url title up duration))))
          (setq info `(,@info :item ,url)))
...
(defvar mpvi-ffmpeg-option-templates
  `(("-ss 0"         . "seeks (in/out)")
    ...
    "-map 0:a"
    "-map 0:s"
    ("-filter/vf/af" . "filter graph, -filter:v, -filter:a (out/s)")
...
(defun mpvi-add-emms (path)
  ...
  (let* ((info (mpvi-extract nil path))
         (items (if-let* ((item (plist-get info :item)))
                    `((,item ,(plist-get info :title) ,(plist-get info :up)))
                  (mapcar #'ensure-list (plist-get info :items)))))
    (unless items (user-error "Unkown source: %s" path))
    (cl-loop for (filename title artist) in items
             for track = (emms-track 'mpvi filename)
             if (and (null title) (string-prefix-p "http" filename)) do (setq title filename)
