...
(defcustom mpvi-init-cmds '(((set_property autofit-larger "60%")))
...
BEG and END for ab-loop play."
  ...
  (let ((seek (lambda ()
                (when (mpvi-seekable)
                  ...
                  (mpvi-set 'pause 'no))))
        (addp (and type (not (memq type '(emms)))))
        (emmsp (emms-playlist-current-selected-track))
        (current (and (emms-player-mpv-proc-playing-p) (mpvi-cmd `(get_property path)))))
    (if (and (equal path current) (if emmsp (eq type 'emms) (not (eq type 'emms))))
        (funcall seek)
      (unless type (message "Waiting %s..." path))
      ...
      (let* ((lowp (ignore-errors (mpvi-compare-mpv-version #'< '(0 38 0))))
             ...
             (items (plist-get info :items)))
        (when (and items (not addp))
          (setq info (append (mpvi-extract nil (completing-read "Item: " items nil t))
                             info)))
        (let* ((item (plist-get info :item))
               (items (if item (list item) items))
               (logo (plist-get info :logo))
               (title (plist-get info :title))
               (subfile (plist-get info :subfile))
               (load-opts (plist-get info :load-opts))
               (start-func (plist-get info :start-func))
               (play-cmd nil))
          (cond ((and item (equal item current))
                 (funcall seek))
                ((and type (not (memq type '(emms))))
                 (let ((flag (if current-prefix-arg
                                 (completing-read "Playlist flag: " '(append append-play insert-next insert-next-play) nil t)
                               (or type (if current 'append 'append-play)))))
                   (setq play-cmd `(batch
                                    ,@(cl-loop for c in items collect `((loadfile ,c ,(intern-soft flag))))))))
                (t
                 (setq load-opts (mapconcat
                                  ...
                                  ","))
                 (setq play-cmd `(batch
                                  ,@(unless (or current (eq type 'emms))
                                  ...
                                  ((loadfile ,item replace ,@(if lowp (list load-opts) (list -1 load-opts))) .
                                   ,(lambda (_ err)
...
(cl-defmethod mpvi-extract ((_ (eql 'url)) path &rest _)
  "Extract PATH for URL."
  (if-let* ((items (mpvi-ytdlp-get-playlist path)))
      (list :origin path :items items)
    (list :origin path :item path)))
...
(defun mpvi-position (val)
  "Jump to new position based on VAL for MPV player.
...
(defun mpvi-ytdlp-get-playlist (url)
  "Return items of playlist for URL."
  (let ((meta (mpvi-ytdlp-url-metadata url)))
    (when (assoc 'is_playlist meta)
      (cl-loop for item across (alist-get 'entries meta)
...
Return (suggestion-save-name . video-format)."
  ...
  (with-temp-buffer
    ...
    (let* ((name (if (equal (mpvi-get 'path) url)
                     (mpvi-get 'media-title)
                   (match-string 1)))
...
;;;###autoload
(defun mpvi-play (&optional append)
  "Play video with MPV player.
Interactively choose a file. Prefer the file or URL at point.
If APPEND, only add the video into playlist of MPV player."
  (interactive)
  (let* ((prompt (if append "Add to playlist (file or url) " "Play video (file or url): "))
    ...
    (mpvi-start path (if append 'append-play))))
...
(defun mpvi-add-emms (path)
  ...
  (let* ((info (mpvi-extract nil path))
         (item (plist-get info :item))
         (items (plist-get info :items))
         (addfn (lambda (c &optional desc)
                  (let ((track (emms-track 'mpvi c))
                        (desc (or desc (if (string-prefix-p "http" c) c))))
                    (if desc (emms-track-set track 'info-title (or desc)))
                    (with-current-emms-playlist
                      (unless (= (line-beginning-position) (point))
                        (forward-line))
                      (emms-playlist-insert-track track))))))
    (cond (item (funcall addfn item))
          (items (mapc addfn items))
  ...
  (if-let* ((current (and (not current-prefix-arg) (emms-playlist-current-selected-track))))
                                        ...
                                        (lambda (input pred action)
                                          (if (eq action 'metadata)
                                              `(metadata
                                                ...
                                                (annotation-function
                                                 . ,(lambda (c)
                                                      (let ((cc (assoc c track-indices)))
                                                        (unless (string-match-p "/" (caddr cc))
                                                          (concat (make-string (max 2 (- 50 (string-width (caddr cc)))) ? )
                                                                  (substring-no-properties (car cc))))))))
    ...
    (let* ((playlist (mpvi-get 'playlist))
           (items (cl-loop with idx = 0
                           for i from 0 for p across playlist
                           for file = (alist-get 'filename p)
                           for title = (or (alist-get 'title p)
                                           (file-name-sans-extension (file-name-nondirectory file)))
                           if (alist-get 'current p) do (setq idx i)
                           collect (list (propertize (format "%s %s" (alist-get 'id p) title) 'display title) file title) into rs
                                  ...
                                  (lambda (input pred action)
                                    (if (eq action 'metadata)
                                        `(metadata
                                          ...
                                          (annotation-function
                                           . ,(lambda (c)
                                                (let ((cc (assoc c items)))
                                                  (concat (make-string (max 2 (- 50 (string-width (caddr cc)))) ? )
                                                          (substring-no-properties (cadr cc)))))))
                                      (complete-with-action action items input pred)))
                                  nil t))
           (id (string-to-number (car (split-string cand))))
           (index (or (cl-position-if (lambda (c) (equal (alist-get 'id c) id)) playlist)
...
(defvar mpvi-control-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "n")   (mpvi-fn1 (mpvi-position "+1s") t))
    (define-key map (kbd "p")   (mpvi-fn1 (mpvi-position "-1s") t))
    (define-key map (kbd "N")   (mpvi-fn1 (mpvi-position "+5s") t))
    (define-key map (kbd "P")   (mpvi-fn1 (mpvi-position "-5s") t))
    (define-key map (kbd "M-n") (mpvi-fn1 (mpvi-position "+1%") t))
    (define-key map (kbd "M-p") (mpvi-fn1 (mpvi-position "-1%") t))
    (define-key map (kbd "C-n") (mpvi-fn1 (mpvi-position "+1s") t))
    (define-key map (kbd "C-p") (mpvi-fn1 (mpvi-position "-1s") t))
    (define-key map (kbd ".")   (mpvi-fn1 (mpvi-position "+1f") t))
    (define-key map (kbd ",")   (mpvi-fn1 (mpvi-position "-1f") t))
    (define-key map (kbd ">")   (mpvi-fn1 (mpvi-position "+1f") t))
    (define-key map (kbd "<")   (mpvi-fn1 (mpvi-position "-1f") t))
    (define-key map (kbd "C-l") (mpvi-fn1 (mpvi-position "-0s") t))
    (define-key map [left]      (mpvi-fn1 (mpvi-position "-5s") t))
    (define-key map [right]     (mpvi-fn1 (mpvi-position "+5s") t))
    ...
    (define-key map (kbd "x")   #'mpvi-cmd)
    (define-key map (kbd "s")   #'mpvi-set)
...
(defun mpvi-control-tips ()
  ...
  (interactive)
  (let ((tips '(("SPC m M T f L" . "Toggle Pause/Mute/Video/Ontop/Fullscreen/AB-Loop")
...
AB-LOOP-B are status of MPV player. WW is window width."
  (let* ((width (min (max 30 (- ww 15)) 60))
         (percent (/ playback-time duration))
...
If ALIST exists, only refresh the property data in it."
  (when (null mpvi-control--refreshing)
            ...
            (if (emms-player-mpv-proc-playing-p)
                (with-current-buffer buf
                  (let* ((f1 (lambda (k) (if-let* ((v (assoc k alist))) (cdr v)
                    ...
                    (and-let* ((vals (funcall fm '(path media-title warning ww))))
                      (apply #'mpvi-control-render-header vals))
                    (and-let* ((keys '(playback-time duration speed volume mute video pause ab-loop-a ab-loop-b ww focus))
...
(cl-defmethod mpvi-event :around (type data)
  "Add some event handlers for event TYPE and its DATA."
  (ignore-errors
    (when (eq type 'start-file)
      (mpvi-async-cmd `(batch
                        ,@(cl-loop for i from 900
                                   for c in '(speed volume ab-loop-a ab-loop-b mute video pause)
                                   collect `((observe_property ,i ,c))))))
    ...
    (when (and (buffer-live-p (get-buffer mpvi-control-buffer))
      ...
      (cond ((eq type 'property-change)
             (let* ((key (intern (alist-get 'name data)))
                    (val (alist-get 'data data)))
               (mpvi-control-refresh (list (cons key val)))))
